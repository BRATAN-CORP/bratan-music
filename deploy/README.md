...
```
   ...
   Internet ──▶  │ Cloudflare Tunnel (cloudflared)
        ...
        └──────┬───────┘     └──────────────┘   └──────────────┘
               │
        ┌──────▼──────────────┐
        │ postgres-backup-tg  │  ежедневный dump → Telegram (admin chat)
        └─────────────────────┘  никаких локальных томов; см. backup-tg/
```
...
1. Билдит образы `bratan-music-api-go` и `bratan-music-web`, пушит в GHCR.
