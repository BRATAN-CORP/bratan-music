...
const SUBSCRIPTION_BENEFITS: { icon: typeof Check; key: TranslationKey }[] = [
  { icon: Sparkles, key: 'profile.benefits.lossless' },
  { icon: Music2, key: 'profile.benefits.hifi' },
  { icon: Shield, key: 'profile.benefits.noads' },
];
    ...
    <AuthGuard>
      <div className="mx-auto flex max-w-6xl flex-col gap-8 p-4 sm:p-6 lg:p-10">
        ...
        <div className="grid gap-4 md:grid-cols-2">
            ...
            {crossfade && (
              <div className="mt-3">
                <label className="flex items-center justify-between text-xs text-muted-foreground">
                  ...
                  <span className="tabular-nums text-foreground">{t('settings.crossfadeDurationUnit', { value: crossfadeDuration })}</span>
                </label>
                <input
                  type="range"
                  min={1}
          ...
          <SettingsCard title={t('settings.qualityProvider')} icon={Music2} hint={t('settings.qualityHint')}>
    ...
    >
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <h1 className="mt-0.5 truncate text-2xl font-semibold tracking-tight sm:text-3xl">
              {name ?? username ?? t('common.anonymous')}
            </h1>
        ...
        >
          ...
          <span className="tabular-nums" data-locale={locale}>{theme === 'dark' ? t('settings.themeDark') : t('settings.themeLight')}</span>
        </button>
      </div>
    </section>
    </TiltCard>
      ...
      <div className="relative grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
        <div className="flex flex-col gap-4">
          ...
          <h2 className="max-w-xl text-2xl font-semibold leading-tight tracking-tight sm:text-4xl">
            {(() => {
              // Render the headline with its emphasised fragment
              // wrapped in a serif-italic span. We split the full
              // headline on the emphasised fragment so translators
              // can move the emphasis to wherever it sits naturally
              // in their language without breaking the markup.
              const full = t('profile.premiumHeadline');
              const emphasis = t('profile.premiumHeadlineEmphasis');
              const idx = full.indexOf(emphasis);
              if (idx === -1) return full;
              const before = full.slice(0, idx);
              const after = full.slice(idx + emphasis.length);
              return (
                <>
                  {before}
...
function AdminGrantPanel() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (/^\d+$/.test(trimmed)) payload.userId = trimmed;
      else payload.tgUsername = trimmed;
      ...
      if (r.ok && r.user && r.subscription) {
        ...
        const exp = new Date(r.subscription.expiresAt * 1000).toLocaleDateString(dateLocale);
        setMsg({ kind: 'ok', text: t('admin.grantSuccess', { target: u, days: r.subscription.days, expiresAt: exp }) });
        setTarget('');
      } else {
        setMsg({ kind: 'err', text: t('admin.grantFailed') });
      }
    } catch (err) {
      setMsg({ kind: 'err', text: err instanceof Error ? err.message : t('admin.errorFallback') });
    } finally {
      setBusy(false);
    }
  };
