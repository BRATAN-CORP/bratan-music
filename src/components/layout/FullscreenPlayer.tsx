      ...
      {fullscreen && currentTrack && (
        ...
        >
          ...
          {(trackCoverUrl || coverVideoUrl) && (
            <>
              {/* Watercolor displacement layer (-z-20) — sits BEHIND the
                  ambient blur layer (-z-10) and gives static covers a
                  continuous, organic flow. The cover is fed through an
                  SVG `feDisplacementMap`; the displacement source is a
                  static `feTurbulence` shifted over time by an animated
                  `feOffset`. The noise itself is computed once per cover
                  load — only the offset changes per frame, which keeps
                  the filter cheap on iOS Safari.
                  • Skipped for animated covers (`coverVideoUrl`) — those
                    already provide motion themselves.
                  • Skipped under `prefers-reduced-motion` — the user has
                    asked the system to minimise non-essential animation.
              */}
              {trackCoverUrl && !coverVideoUrl && !reduce && (
                <>
                  <svg
                    aria-hidden
                    width="0"
