  ...
  const submit = async (isAdmin: boolean) => {
    ...
    try {
      ...
      if (r.ok && r.user) {
        ...
        setMsg({
          kind: 'ok',
          text: t(r.user.isAdmin ? 'admin.flagPromoted' : 'admin.flagDemoted', { user: u }),
        });
        setTarget('');
      } else {
        setMsg({ kind: 'err', text: r.error ?? t('admin.flagFailed') });
      }
    } catch (err) {
      ...
      <div className="mt-3 flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Button onClick={() => submit(true)} disabled={busy || !target.trim()} className="flex-1">
            {busy ? t('common.applying') : t('admin.flagPromote')}
          </Button>
          <Button
            variant="outline"
