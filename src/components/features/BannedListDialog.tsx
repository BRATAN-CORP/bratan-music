...
export function BannedListDialog({ open, onClose, kind }: Props) {
  ...
  return (
    <Modal
      ...
      align="sheet"
      labelledBy={titleId}
      panelClassName="max-w-[560px] flex flex-col rounded-[var(--radius-xl)] sm:rounded-[var(--radius-lg)] max-h-[calc(100dvh-7rem-env(safe-area-inset-bottom,0px))]"
    >
      <header className="flex items-center justify-between border-b border-border/60 px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          <Ban size={15} className="text-muted-foreground" />
          <span id={titleId} className="truncate text-sm font-medium">{t(titleKey)}</span>
