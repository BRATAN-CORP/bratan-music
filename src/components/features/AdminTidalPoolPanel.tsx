...
import {
  AlertCircle, CheckCircle2, KeyRound, Loader2, Plus, RefreshCw,
  Server, ShieldCheck, ShieldOff, Trash2, X,
} from 'lucide-react';
import { Button } from '@/components/ui/Button';
import { api } from '@/lib/api';

/**
 * Tidal pool admin panel.
 *
 * Lists every account the worker knows about with subscription / health
 * info, and lets the admin add new accounts via refresh token, toggle
 * individual ones on or off, rename them, and remove them. The legacy
 * single-account `<AdminTidalPanel />` (the device-flow + status box)
 * still lives next to this and shares the same backend — it just
 * targets the picker default rather than a specific account.
 */
...
export function AdminTidalPoolPanel() {
    ...
    <section className="rounded-[var(--radius-md)] border border-border bg-card p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Распределение нагрузки между несколькими прокси-аккаунтами Tidal. Воркер выбирает наименее
            нагруженный включённый аккаунт; аккаунт с 5+ ошибками подряд автоматически выключается.
          </p>
        </div>
      ...
      <AnimatePresence initial={false}>
        {showAdd && (
          ...
          >
            <div className="mt-4 flex flex-col gap-2 rounded-[var(--radius-sm)] border border-border bg-background p-3">
              ...
              <div className="flex items-center justify-between gap-2">
                <p className="text-[11px] text-muted-foreground">
                  Worker сразу обменяет refresh token на access token и подтянет тип подписки. Если
                  аккаунт с тем же Tidal user id уже есть, его токены обновятся.
                </p>
                <Button onClick={addAccount} disabled={adding || !refreshTokenInput.trim()} size="sm">
      ...
      {!loading && accounts && accounts.length === 0 && (
        <p className="mt-6 rounded-[var(--radius-sm)] border border-dashed border-border bg-background p-6 text-center text-xs text-muted-foreground">
          Пул пуст. Добавь первый аккаунт через форму выше или через единичную секцию ниже.
        </p>
      )}
