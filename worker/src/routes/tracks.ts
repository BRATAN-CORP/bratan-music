 ...
 * Returning an empty list there is bad UX — the "Similar" section on
 * /track/:id silently disappears for half the catalogue.
 *
 ...
 *      Tidal-known artist, even when both radios are gated).
 *   4. Sibling tracks from the same album, only if we still have
 *      < 5 items — cheap final padding so the section never looks
 *      embarrassingly thin.
 *
 * We dedupe across all sources and exclude the seed track itself.
...
tracks.get('/:id/radio', async (c) => {
  ...
  // 1. Tidal track radio. Short-circuit if it returns a usable mix.
  try {
    const seed = await tidal.getTrackRadio(id);
    pushUnique(seed);
  ...
  // We need the seed track's artist/album for the fallback layers.
