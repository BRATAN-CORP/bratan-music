...
export class AiPlaylistService {
  ...
  private async askForPlan(prompt: string, want: number): Promise<AiPlan> {
    const system = [
      'Ты — музыкальный куратор, который помогает выбирать треки в Tidal.',
      'Тебе дают промпт на русском или английском, ты должен превратить его в JSON-план.',
      'Ты НЕ знаешь конкретный каталог Tidal и НЕ должен придумывать названия треков или артистов «из головы».',
      'Вместо этого собери список поисковых запросов (queries), которые сервис прогонит через Tidal Search.',
      'Хорошие запросы: короткие (1–4 слова), очень специфические, без знаков препинания.',
      'Можно микшировать запросы по жанрам, эпохам, настроениям, конкретным артистам.',
