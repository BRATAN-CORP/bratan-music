...
| `useLyrics` | Lyrics fetch + sync. Reads `OfflineTrack.lyrics` first when the track is saved on-device, falls back to network, and back-fills the offline row when the network response arrives — so saved tracks render in PWA / offline mode. |
