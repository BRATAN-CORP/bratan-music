...
| 10 | `devin/1778794183-recs-country-genre-vector` | **Recs: country/language signal + genre vector + dial weights (user 1+2+5).** Симптом — Polish rap пролезает в волну при 100% RU/EN истории. Fix: (а) `TasteProfile` расширен `scriptMix: {cyrillic, latin, cjk, other}` (нормализованная доля каждой системы письма в `play_history.artist_name` с тем же экспоненциальным декаем что и `artistWeights`) + `genreWeights: Record<slug, [0..1]>` от seed-жанров и (future) per-artist genre lookup; версия профиля 1 → 2 (старые кэши перезаписываются на следующем `getOrCompute`); (б) `RecommendationService.rerank` получает 2 новых сигнала — `W_GENRE_MATCH=0.20` бонус когда трек пришёл из explore-slug совпадающего с топ-жанрами юзера (через cross-call `genreProvenance: Map<trackKey, slug>`), и `W_LANG_MISMATCH=-0.40` штраф на латиницу/cjk/other когда доля скрипта в истории <10% (gated `totalPlays >= LANG_MIN_HISTORY_PLAYS=50` чтобы cold-start не лочило в один регион); (в) ручка весов крутнута: `W_TASTE` 0.55 → 0.85, `W_NOVELTY` 0.20 → 0.10 чтобы taste-сигнал не тонул в `Math.random()` для незнакомых артистов с `tasteSig=0`; (г) экстракт shared helper `trackKey(t)` чтобы wave/rerank/seen/mood/provenance все ходили через одну строку. | open | (этот PR) |
